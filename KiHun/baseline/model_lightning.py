...
class EASTLightningModel(pl.LightningModule):
    def __init__(self, image_size=2048, input_size=1024, learning_rate=1e-3, max_epoch=150):
        super().__init__()
        self.model = EAST()
    ...
    def on_train_epoch_end(self):
        ...
        self.log_dict({"Mean loss": mean_loss})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred_score_map, pred_geo_map = extra_info["score_map"], extra_info["geo_map"]
    ...
    def on_validation_epoch_end(self):
        ...
        precision, recall, f1 = np.array(self.val_precisions).mean(), np.array(self.val_recalls).mean(), np.array(self.val_f1s).mean()
        metrics = {'val_precision': precision, 'val_recall': recall, 'val_f1': f1}

        # wandb와 로그에 기록
        self.log_dict(metrics)

        self.val_precisions.clear(), self.val_recalls.clear(), self.val_f1s.clear()
